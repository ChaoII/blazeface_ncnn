...
static void
nms_sorted_bboxes(const std::vector<FaceObject> &faceobjects, std::vector<int> &picked, float nms_threshold) {
    picked.clear();
...
int BlazeFace::generate_anchors(int target_size, int step_size, std::vector<float> min_sizes,
                                std::vector<float> aspect_ratios, float offset, std::vector<float> variances) {
...
void BlazeFace::generate_proposals(const ncnn::Mat &score_blob, const ncnn::Mat &bbox_blob, float score_threshold,
...
int BlazeFace::load(int _target_size, bool use_gpu) {
    blazeface.clear();
    ...
    blazeface.opt.num_threads = 8;

    blazeface.load_param("../model/blazeface.param");
    blazeface.load_model("../model/blazeface.bin");
...
int BlazeFace::detect(const cv::Mat &rgb, std::vector<FaceObject> &faceobjects, float prob_threshold,
                      float nms_threshold) {
    int width = rgb.cols;
    int height = rgb.rows;
